...
//}
/**
 * Old version has an independent pipeline, new version pulls the pipeline from the current part that is being picked
 * that is whatever pipeline is tied to the fiducial. This makes moding things way easier.
 * TODO: In the new code, when editing a pipeline, it should ask the user if it would like to move
 *       to the next pick location automatically to help with the pipeline editing.
 *
 *
 */
////////////////////////////////////Above is working og code -- below is new code//////////
/*
 * Copyright (C) 2025 Your Name
 *
 * This file is part of OpenPnP.
...
public class JEDEC_TrayFeederConfigurationWizard extends AbstractConfigurationWizard {
    ...
    // New: labels inside the Vision "context" group box
    private JLabel lblPartValue;
    private JLabel lblPipelineValue;
    ...
    public JEDEC_TrayFeederConfigurationWizard(JEDEC_TrayFeeder feeder) {
        ...
        visionPanel.setLayout(new FormLayout(new ColumnSpec[] {
                ...
                FormSpecs.DEFAULT_COLSPEC, FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC, },
                new RowSpec[] {
                        FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC,               // row 2: context group
                        FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC,               // row 4: feed/fid row
                        FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC                // row 6: training row
                }));

        // Sub group box for context: Part & Pipeline
        JPanel contextPanel = new JPanel();
        contextPanel.setBorder(new TitledBorder(null, "Current Part & Fiducial Pipeline",
        ...
        JLabel lblPartLabel = new JLabel("Part:");
        contextPanel.add(lblPartLabel, "2, 2");
        lblPartValue = new JLabel("(none)");
        contextPanel.add(lblPartValue, "4, 2");

        JLabel lblPipelineLabel = new JLabel("Pipeline:");
        contextPanel.add(lblPipelineLabel, "2, 4");
        lblPipelineValue = new JLabel("(none)");
        contextPanel.add(lblPipelineValue, "4, 4");

        visionPanel.add(contextPanel, "2, 2, 5, 1");

        JLabel lblFeedPipeline = new JLabel("Feed / Fiducial Pipeline (per Part)");
        ...
        JLabel lblTrainingPipeline = new JLabel("Training Pipeline (Feeder-local)");
        ...
        // Update context when user changes part
        comboBoxPart.addActionListener(e -> updatePipelineContextLabels());
    }
    ...
    public void createBindings() {
        ...
        // Initialize context labels based on current part / settings
        updatePipelineContextLabels();
    }
    ...
    // ---------- Helper: update context group (Part & Pipeline) ----------
    private void updatePipelineContextLabels() {
        if (lblPartValue == null || lblPipelineValue == null) {
            return;
        }
        Part part = feeder.getPart();
        if (part == null) {
            lblPartValue.setText("(none)");
            lblPipelineValue.setText("(none)");
            return;
        }

        // Part name (fallback to ID)
        String partText = part.getName();
        if (partText == null || partText.trim().isEmpty()) {
            partText = part.getId();
        }
        lblPartValue.setText(partText);
        ...
        try {
            ...
            if (fvs == null) {
                lblPipelineValue.setText("(no fiducial vision configured)");
                return;
            }

            String name = null;

            // Prefer FVS name if present
            try {
                java.lang.reflect.Method getName = fvs.getClass().getMethod("getName");
                Object nameObj = getName.invoke(fvs);
                if (nameObj != null) {
                    name = String.valueOf(nameObj);
                }
            }
            catch (NoSuchMethodException ignored) {
                // Fall back to pipeline's name
                try {
                    java.lang.reflect.Method getPipelineMethod = fvs.getClass().getMethod("getPipeline");
                    Object pipelineObj = getPipelineMethod.invoke(fvs);
                    if (pipelineObj != null) {
                        java.lang.reflect.Method getName2 = pipelineObj.getClass().getMethod("getName");
                        Object nameObj2 = getName2.invoke(pipelineObj);
                        if (nameObj2 != null) {
                            name = String.valueOf(nameObj2);
                        }
                    }
                }
            ...
            lblPipelineValue.setText(name);
        }
        catch (NoSuchMethodException e) {
            lblPipelineValue.setText("(fiducial vision not supported)");
        }
        catch (Exception e) {
            lblPipelineValue.setText("(error reading pipeline)");
        }
    }
    ...
    private void editPipeline() throws Exception {
        ...
        try {
            java.lang.reflect.Method getFvs = part.getClass().getMethod("getFiducialVisionSettings");
            fvs = getFvs.invoke(part);
        }
        catch (NoSuchMethodException e) {
            throw new Exception("Part " + part.getId() + " does not support FiducialVisionSettings in this OpenPnP version.");
        }

        if (fvs == null) {
            throw new Exception("Part " + part.getId() + " has no Fiducial Vision Settings configured.");
        }

        Object pipelineObj;
        try {
            java.lang.reflect.Method getPipelineMethod = fvs.getClass().getMethod("getPipeline");
            pipelineObj = getPipelineMethod.invoke(fvs);
        }
        catch (NoSuchMethodException e) {
            throw new Exception("FiducialVisionSettings for part " + part.getId() +
                    " has no getPipeline() method; cannot edit pipeline here.");
        }

        if (!(pipelineObj instanceof CvPipeline)) {
            throw new Exception("Unsupported FiducialVisionSettings pipeline type: " +
                    (pipelineObj == null ? "null" : pipelineObj.getClass().getName()));
        }

        CvPipeline pipeline = (CvPipeline) pipelineObj;
        pipeline.setProperty("camera", Configuration.get().getMachine().getDefaultHead().getDefaultCamera());
        pipeline.setProperty("feeder", feeder);
        pipeline.setProperty("part", part);
        CvPipelineEditor editor = new CvPipelineEditor(pipeline);
        JDialog dialog = new CvPipelineEditorDialog(MainFrame.get(), part.getId() + " Fiducial Pipeline", editor);
        dialog.setVisible(true);

        // Refresh context (pipeline name might have changed)
        updatePipelineContextLabels();
    }
    ...
    private void resetPipeline() {
        ...
        if (part == null) {
            MessageBoxes.errorBox(getTopLevelAncestor(), "Error",
                    "Feeder " + feeder.getName() + " has no part.");
            return;
        }
        try {
            java.lang.reflect.Method getFvs = part.getClass().getMethod("getFiducialVisionSettings");
            ...
            if (fvs == null) {
                MessageBoxes.errorBox(getTopLevelAncestor(), "Error",
                        "Part " + part.getId() + " has no Fiducial Vision Settings configured.");
                return;
            }

            try {
                java.lang.reflect.Method reset = fvs.getClass().getMethod("resetPipeline");
                reset.invoke(fvs);
            }
            catch (NoSuchMethodException e) {
                // Older version: no reset on vision settings; fall back to feeder-local reset
                feeder.resetPipeline();
            }
        }
        catch (NoSuchMethodException e) {
            // No fiducial vision support; again fall back
            feeder.resetPipeline();
        }
        catch (Exception e) {
            MessageBoxes.errorBox(getTopLevelAncestor(), "Error",
                    "Error resetting fiducial pipeline: " + e.getMessage());
        }
        // Update context after reset
        updatePipelineContextLabels();
    }
